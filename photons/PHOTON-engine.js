...
var PHOTON = {
  ...
  // Curve detail (not in original code, 64 was just always used)
  _CURVE_DETAIL: 256,
    ...
    switch(m.type) {
      ...
      case 'mirror':
        mirror = new THREE.Mirror(PHOTON.renderer, PHOTON.camera, {
          color: m.color === 10070715?0x5F5F6F:m.color
        });  // Made mirror kind of blue so you can actually see it w/ the white background.
  ...
  LightPath3D: function(x1, z1, x2, z2) {
    ...
    var pathMaterial = { // Material
      ...
      inputs: {
        color: 0xff0000,
        transparent: true,
        opacity: 1  // Changed from opacity: 0.4
      }
    }
  ...
  Polarizer: function() {
    ...
    ,   lensMaterialObj = PHOTON._TEXTURE_SUPPORT?
      {
        type: 'lambertian',  // Changed from type: reflective,
        inputs: {color: 0x333333, transparent: true, opacity: 0.8}  // Changed from color: 0x333333
      } :
      {
        type: 'lambertian',
  ...
  Mirror3D: function() {
    ...
    var mirror = new PHOTON.ThreeChild(
      ...
      { // Material
        type: 'phong',  // Was initially mirror;
        input: {color: 0xb4c1ec, shininess: 50, specular: 0x999999}  // MARKED
      }
    );
    meshes.push(mirror);
